# DSA Python-style pseudocode prompt
...
- Prefer short domain names in pseudocode such as `rooms`, `tasks`, `routes`, `jobs`, `history`, `next_stops`, or `pending_work` instead of meaningless `A`, `B`, `x`, `state1` when a natural real-world name exists.
...
SNIPPET 1 — PSEUDOCODE EXAMPLE
- Output ONE fenced `python` code block for readable syntax highlighting.
- The contents must be Python-style PSEUDOCODE, not runnable Python.
- Prefer clarity of the algorithm over exact Python syntax or library APIs.
- Use simple Python-like assignments, lists, loops, conditions, indentation, and comments.
- Natural pseudocode expressions are encouraged when they make the mechanism clearer, for example:
  - `for each index i, height in heights:`
  - `while stack is not empty and height at stack.top < height:`
  - `stack.push(i)`
  - `old = stack.pop()`
  - `queue.enqueue(item)` / `queue.dequeue()`
- Do NOT add imports, classes, helper boilerplate, or language-specific implementation details unless they are essential to understanding the DSA.
- Keep it compact and visually easy to trace by hand.
...
- Comments must explain the stored state, invariant, deferred work, or decision point — not merely restate the pseudocode.
...
- Show the final result only if it helps understanding.
...
Make the signals concrete enough that a learner can recognize the pattern from problem wording or required operations.
...
- Previous values stay unresolved until a later value satisfies a comparison.
- One new value may resolve several recent unresolved values at once.

Do not add `What to notice`, `Complexity`, or a conclusion.

SNIPPET 3 — FULL DRY RUN
Immediately after Snippet 2, output ONE separate fenced `text` code block that dry-runs the exact pseudocode example from Snippet 1.

Use a compact execution-trace style like this:
- Begin with `START`.
- Show the important initial data structures directly underneath.
- Then show each loop iteration or processing step in execution order using the loop/index/current-value variables from the pseudocode, for example `i = 2, size = 6` or `room = kitchen`.
...
The trace should read like a human manually stepping through the pseudocode, not like a generic logging template.
...
- Follow the conceptual operations in the pseudocode exactly even though the pseudocode is not executable Python.
- Do NOT use a Markdown table.
- Do NOT omit duplicate/skip/continue iterations if they really occur in the pseudocode.
...
```python
# Python-style pseudocode with light real-world flavor
# concise, conceptual, and easy to trace by hand
```
