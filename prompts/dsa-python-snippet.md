...
- Use simple, valid, runnable Python 3.
- Prefer basic Python syntax and the simplest standard-library tool that fits the DSA.
- Avoid classes, helper functions, clever one-liners, comprehensions, decorators, type hints, or abstractions unless they are genuinely needed.
- Keep the control flow visible: simple loops, `if`, `while`, `append`, `pop`, indexing, dictionaries, sets, `deque`, or `heapq` as appropriate.
- Keep it compact and easy to trace by hand.
...
```python
# simple runnable Python example with light real-world flavor
